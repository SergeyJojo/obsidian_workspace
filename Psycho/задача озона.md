...
    password = "<password>" // нельзя передавать переменные окружения в коде потому что это не безопасно, могут получить все доступ у кого есть доступ в гиту
...
    fmt.Println("Connected!") // логировать отдельной библиотекой логи
